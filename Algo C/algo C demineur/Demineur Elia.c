    ...
    for (int lign = 0; lign < size; ++lign) {
        ...
        for (int column = 0; column < size; ++column) {
            if (grid.tiles[lign][column].flag == 1) {
                ...
                printf(" | \x1B[3%dm%d \x1B[0m", (grid.tiles[lign][column].BombAround+1), grid.tiles[lign][column].BombAround);
            }
...
int main() {
    ...
    myGrid.Bomb = GetIntInRange(1, myGrid.size);
    srand(time(NULL));
    int firststart = 1;
    int loose = 1;
